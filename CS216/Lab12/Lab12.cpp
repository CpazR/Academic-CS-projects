    ...
    while (true)
    {
        ...
        else
        {
			...
			for (int i = 0; i < distance.size(); i++)
			{
				...
				{
					if (distance[i] != -1){
						cout << "The distance from the source vertex " << source << " to vertex " << i << " is " << distance[i] << "-edge away." << endl;
                        cout << "The path back from source vertex is:" <<endl;

                        int n=0, tmp=i;
                        cout << tmp;
                        cout << " --> ";
                        while(tmp !=source|| n>=10){//n>10 in case of inf loop
                            n++;
                            tmp=go_through[tmp];
                            cout << tmp;
                            if (tmp != source)
                        ...
                        cout << endl;
                        if (n==10)
                            cout<<"ERROR: TOO MANY LOOPS, CHECK ALGORITHM" << endl;
                    }
                    else
